...
| v0.7 | Road to 1.0: API audit, fencing, and the last breaking-change window | ✅ shipped in 0.7.0 — [`docs/api-audit-1.0.md`](docs/api-audit-1.0.md) classifies the whole public surface (51 filters, 1 action, 25 constant names, 10 subcommands, 4 interfaces, ~70 public statics) with a freeze verdict each (PR #9); ~40 non-allowlist statics marked `@internal` and four multi-site filters centralized behind one application point (PR #10); a real-WordPress integration harness plus a PHP 8.1–8.5 × WP 6.0+ matrix (PR #11); then the renames — 6 filters renamed, the 8 per-module `*_enabled` toggles replaced by one generic `upsun_module_enabled` covering all 13 (with `upsun_integration_enabled`/`upsun_fetcher_enabled` for symmetry), every old name shimmed until 1.0, and `upsun_cloudflare_restore_remote_addr` removed outright |
| v0.8 | Security review, threat model, and the 1.0 documentation set | ✅ shipped in 0.8.0 — a deliberate pass over the four privileged surfaces (vendoring engine, Cloudflare origin guard, DB-writing sanitizers, SMTP): fixed a transport downgrade where an https package download could redirect to http (the bytes become committed, executed code), bounded download and expansion size, and hardened the work directory; [`SECURITY.md`](SECURITY.md) and [`docs/threat-model.md`](docs/threat-model.md) state the guards and the accepted risks; and the documentation 1.0 needs — [`docs/api-reference.md`](docs/api-reference.md) generated from the source with a drift-guard test, [`CHANGELOG.md`](CHANGELOG.md), [`CONTRIBUTING.md`](CONTRIBUTING.md) |
| v1.0 | Freeze the public API | ✅ shipped in 1.0.0 — the surface classified in the 0.7 audit is now frozen under a published [deprecation policy](docs/api-reference.md#deprecation-policy); the 0.7 rename shims and `Upsun\Deprecations` are removed; `Upsun\purge_paths()` ships as the pluggable shared-cache facade (Cloudflare-backed, `upsun_purge_backends` for the rest), closing the last item this roadmap held open; and the README carries an explicit IN/OUT scope statement so the non-goals are documented rather than implied |
| — | Second real consumer | ✅ `artetecha/pergrazia` migrated to the starter model and consumes the package from Packagist through the public API only — 27 lines of site wiring, one filter, nothing pushed upstream. Verified live on its preview: 14/14 `wp upsun doctor` checks pass, migrations applied, preview safety wired, and the headers confirmed at the real edge (`x-platform-cache: HIT`, `noindex`, baseline security headers, no HSTS on non-production HTTPS). The vendoring **write** path remains exercised by KEDS only — pergrazia vendors no premium packages |
